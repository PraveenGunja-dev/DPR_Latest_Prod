...
import { Users, FolderPlus, Settings, FileText, BarChart3, ShieldCheck, Activity, Layers } from 'lucide-react';
...
export const SuperAdminTabs: React.FC<SuperAdminTabsProps> = ({
  ...
  const isAdmin = displayRole === 'Super Admin' || displayRole === 'admin';
  const isPMAG = displayRole === 'PMAG';
    ...
    <TabsList className="flex w-full overflow-x-auto overflow-y-hidden justify-start h-auto p-1 bg-muted/50 scrollbar-hide">
      ...
      {isAdmin && (
        <TabsTrigger
          value="master-groups"
          className="flex items-center gap-2"
          onClick={() => onTabChange("master-groups")}
        >
